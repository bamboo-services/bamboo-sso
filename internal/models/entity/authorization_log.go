...
// AuthorizationLogEntity 表示SSO授权验证日志实体，记录所有授权码验证行为。
//
// 字段说明：
...
//   - VerifiedAt: 验证时间。
//   - CreatedAt: 创建记录的时间戳。
type AuthorizationLogEntity struct {
	...
	// 关联关系
	AuthorizationCode *AuthorizationCodeEntity `json:"authorization_code,omitempty" gorm:"foreignKey:AuthorizationCodeUUID;references:UUID;comment:关联授权码"`
	Application       *ApplicationEntity       `json:"application,omitempty" gorm:"foreignKey:ApplicationUUID;references:UUID;comment:关联应用"`
	User              *UserEntity              `json:"user,omitempty" gorm:"foreignKey:UserUUID;references:UUID;comment:关联用户"`
}

// BeforeCreate 在创建 AuthorizationLogEntity 记录前自动生成新的 UUID（如果当前 UUID 为空）。
func (al *AuthorizationLogEntity) BeforeCreate(tx *gorm.DB) (err error) {
	if al.UUID == uuid.Nil {
