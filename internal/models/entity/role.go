...
// RoleEntity 表示系统中的角色实体，用于定义权限和业务角色。
//
// 字段说明：
...
type RoleEntity struct {
	...
	Name        string    `json:"name" gorm:"type:varchar(100);not null;uniqueIndex;comment:角色名称"`
...
// BeforeCreate 在创建 RoleEntity 记录前自动生成新的 UUID（如果当前 UUID 为空）。
...
// BeforeUpdate 在更新 RoleEntity 记录前自动更新 UpdatedAt 字段。
func (r *RoleEntity) BeforeUpdate(tx *gorm.DB) (err error) {
